...
class InstituicaoEnsinoController {
  static validarId(id) {
    ...
    if (!Number.isInteger(num) || num <= 0) {
      throw new Error('ID de instituição inválido');
    }
    return num;
  }

  static validarCamposObrigatorios({ nome, endereco, cnpj }) {
  ...
  static validarCnpj(cnpj) {
    ...
    if (cnpjLimpo.length !== 14) {
      throw new Error('CNPJ inválido. Um CNPJ válido deve conter 14 dígitos.');
    }
    return cnpjLimpo;
  }
  ...
  static async cadastrar(req, res) {
    try {
      ...
      InstituicaoEnsinoController.validarCamposObrigatorios({ nome, endereco, cnpj });
      const cnpjLimpo = InstituicaoEnsinoController.validarCnpj(cnpj);

      const instituicaoExistente = await InstituicaoEnsinoModel.buscarPorCnpj(cnpjLimpo);
      ...
      return res.status(500).json({
        erro: error.message || 'Falha ao cadastrar instituição de ensino. Por favor, tente novamente mais tarde.'
      });
    }
  }
  ...
  static async buscarPorId(req, res) {
    try {
      const { id } = req.params;
      const idValido = InstituicaoEnsinoController.validarId(id);

      const instituicao = await InstituicaoEnsinoModel.buscarPorId(idValido);
      if (!instituicao) {
  ...
  static async atualizar(req, res) {
    try {
      ...
      const instituicaoExistente = await InstituicaoEnsinoModel.buscarPorId(idValido);
      if (!instituicaoExistente) {
      ...
      if (cnpjLimpo !== instituicaoExistente.cnpj) {
        const cnpjExistente = await InstituicaoEnsinoModel.buscarPorCnpj(cnpjLimpo);
        if (cnpjExistente) {
          return res.status(400).json({
      ...
      await InstituicaoEnsinoModel.atualizar(idValido, {
        nome,
        endereco,
        cnpj: cnpjLimpo,
        telefone
      ...
      const instituicaoAtualizada = await InstituicaoEnsinoModel.buscarPorId(idValido);
  ...
  static async excluir(req, res) {
    try {
      ...
      await InstituicaoEnsinoModel.excluir(idValido);
      ...
      if (error.code === 'ER_ROW_IS_REFERENCED_2') {
        return res.status(400).json({
          erro: 'Esta instituição não pode ser excluída porque existem alunos vinculados a ela. Transfira os alunos para outra instituição primeiro.'
        });
      }
